...
`__TAURI_INTERNALS__` は Tauri の内部実装の詳細である。Tauri v2 のリリース間で事実上安定しているが、パブリック API の一部ではない。より公式なチェックが必要な場合は、Tauri v2.4+ の `@tauri-apps/api/core` パッケージが公式のパブリック代替として `isTauri()` を提供している。

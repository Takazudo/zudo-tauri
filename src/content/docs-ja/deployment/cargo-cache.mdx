...
## 前提条件: beforeBuildCommand

この記事は `tauri.conf.json` に `beforeBuildCommand` が設定されており、`cargo tauri build` が Rust コンパイル前にフロントエンドを実際にリビルドすることを前提としている。`beforeBuildCommand` が未設定の場合、問題は Cargo キャッシュではなく、そもそもフロントエンドがリビルドされていないことにある。設定の詳細は[アプリバンドルのビルド](./build-bundle.mdx)を参照。
...
これは根本原因を修正する主要な解決策である。ほとんどの場合、`rerun-if-changed` の追加で十分であり、以下の回避策はスキップできる。

ただし、`rerun-if-changed` は**ディレクトリエントリ**を監視する。Cargo の変更検出がトリガーされない場合（ディレクトリの mtime が変わっていない、またはインクリメンタルコンパイルが再リンク不要と判断した場合など）、フォールバックとして `cargo clean -p` が必要になることがある。フロントエンド変更後にビルドが 5 秒未満で完了した場合、Cargo が再コンパイルをスキップした可能性が高い。
...
`--release` フラグが重要である。これがないと `cargo clean -p` は debug 成果物のみをクリーンし、`cargo tauri build` が使用する release ビルドはクリーンされない。

...
`__TAURI_INTERNALS__` is an internal Tauri implementation detail. It has been stable in practice across Tauri v2 releases, but it is not part of the public API. If you need a more official check, Tauri's `@tauri-apps/api/core` package exposes `isTauri()` in v2.4+ as a proper public alternative.
